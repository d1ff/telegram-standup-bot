...
class MongoStorage(BaseStorage):
    ...
    def __init__(self, host='localhost', port=27017, db_name='aiogram_fsm',
        ...
        self._mongo: Union[AsyncIOMotorClient, None] = None
        self._db: Union[AsyncIOMotorDatabase, None] = None
    ...
    async def get_states_list(self) -> List[Tuple[int, int]]:
        ...
        items = await db[STATE].find().to_list(None)
        for item in items:
            result.append(
    ...
    async def get_data_list(self) -> List[Tuple[int, int]]:
        """
        Get list of all stored chat's and user's
        ...
        items = await db[DATA].find().to_list(None)
        for item in items:
            result.append(
                (int(item['chat']), int(item['user']))
            )
